...
REGION = os.environ.get("REGION", "dummy-region")
ALLOYDB_DATABASE_NAME = os.environ.get("ALLOYDB_DATABASE_NAME", "dummy-database-name")
ALLOYDB_TABLE_NAME = os.environ.get("ALLOYDB_TABLE_NAME", "dummy-table-name")
ALLOYDB_CLUSTER_NAME = os.environ.get("ALLOYDB_CLUSTER_NAME", "dummy-cluster-name")
ALLOYDB_INSTANCE_NAME = os.environ.get("ALLOYDB_INSTANCE_NAME", "dummy-instance-name")
ALLOYDB_SECRET_NAME = os.environ.get("ALLOYDB_SECRET_NAME", "dummy-secret-name")
...
try:
    ...
    print(f"Warning: Could not import Google Cloud libraries: {e}")
    IMPORTS_AVAILABLE = False

# Try to initialize Google Cloud services with error handling
if IMPORTS_AVAILABLE:
    try:
        ...
        print("Running in demo mode without Google Cloud connectivity")
        vectorstore = None
else:
    print("Running in demo mode - Google Cloud libraries not available")
...
def create_app():
    ...
    def talkToGemini():
        try:
            if vectorstore is None or not IMPORTS_AVAILABLE:
                # Demo mode response
                request_data = request.get_json() or {}
                user_message = request_data.get('message', 'No message provided')
                return {
                    "content": f"Demo mode: Shopping assistant received your message '{user_message}'. In production, this would provide AI-powered shopping recommendations based on room analysis and product catalog.",
                    "details": {"mode": "demo", "message": user_message}
                }
            
            # Original functionality when Google Cloud services are available
            ...
            print(f"Error in talkToGemini: {str(e)}")
            return {"content": f"Error: {str(e)}", "details": {"error": True}}, 500

    @app.route("/health", methods=['GET'])
    def health():
        return {"status": "healthy", "service": "shopping-assistant", "mode": "production" if vectorstore else "demo"}
...
if __name__ == "__main__":
    ...
    app = create_app()
    print(f"Starting Shopping Assistant Service on port 8005")
    print(f"Project ID: {PROJECT_ID}")
    print(f"Mode: {'Production' if vectorstore else 'Demo'}")
